...
(s.categories && s.categories.length) ? s.categories : structuredClone(DEFAULT_SETTINGS.categories);
      s.budgets = 
...
(err);
    ...
    appEl.appendChild(U.el("div",{class:"card"},[
      U.el("div",{class:"h1",text:"Error iniciando 

la app"}),
      U.el("div",{class:"tiny muted", style:"margin-top:8px"}, "No se ha podido abrir la base de datos del navegador 

(IndexedDB). Prueba en otro navegador o revisa permisos.")
    ]));
    loadingEl?.remove();
    return;
  }
...
"#/panel").replace("#/","");
    const ok = ["panel","movimientos","presupuesto","cuentas","ajustes"].includes(h) ? h : 
...
(state.db, startMs, endMs)
    ]);
    // sort by date desc
    tx.sort((a,b)=>b.dateMs - a.dateMs);
    tr.sort((a,b)=>b.dateMs - 
...
([DB.listAllTx(state.db), DB.listAllTr(state.db)]);
    tx.sort((a,b)=>b.dateMs - a.dateMs);
    tr.sort((a,b)=>b.dateMs - 
...
(t.type==="income") income += Number(t.amount||0);
...
helpers ----------
  function sectionHeader(title, rightNode=null){
    return U.el("div",{class:"row space", style:"margin-
...
("button",{class:"btn small", text:"◀", onclick: ()=>{ state.periodOffset -= 1; render(); }}),
      U.el("div",{class:"badge", text: 
...
exportBtn(onclick, text="Exportar XLSX"){
    return U.el("button",{class:"btn small", text, onclick});
  }

  function printBtn

(onclick, text="Imprimir"){
    return U.el("button",{class:"btn small", text, onclick});
  }
...
Transferencia) ----------
  function openQuickAddModal(){
    ...
    const content = U.el("div",{class:"grid", 

style:"gap:10px"},[
      U.el("div",{class:"tiny muted", text:"¿Qué quieres añadir?"}),
      U.el("button",{class:"btn primary", 

text:"Gasto", onclick: ()=>{ modal?.close(); openTxModal({mode:"add", tx:null, _forceType:"expense"}); }}),
      U.el

("button",{class:"btn primary", text:"Ingreso", onclick: ()=>{ modal?.close(); openTxModal({mode:"add", tx:null, 

_forceType:"income"}); }}),
      U.el("button",{class:"btn", text:"Transferencia entre cuentas", onclick: ()=>{ modal?.close(); 
...
tr?.fromAccountId || "bank";
    toSel.value = tr?.toAccountId || "cash";

    const amountInp = U.el("input",{class:"input", 

placeholder:"Importe (ej: 50,00)", inputmode:"decimal"});
...
("div",{},[U.el("div",{class:"tiny muted",text:"Desde"}), fromSel]),
      U.el("div",{},[U.el("div",{class:"tiny 

muted",text:"Hacia"}), toSel]),
      U.el("div",{},[U.el("div",{class:"tiny muted",text:"Importe"}), amountInp]),
      U.el

("div",{},[U.el("div",{class:"tiny muted",text:"Nota"}), noteInp]),
    ]);

    const saveBtn = U.el("button",{class:"btn primary", 

text: isEdit ? "Guardar" : "Transferir"});
...
cancelBtn.className = "btn";
    ...
    const modal = U.openModal({
      title: isEdit ? "Editar transferencia" : "Transferencia entre 
...
saveBtn.onclick = async ()=>{
      const amount = U.parseAmount(amountInp.value);
      if(!amount || amount<=0){ U.toast

("Importe no válido."); amountInp.focus(); return; }
      if(fromSel.value === toSel.value){ U.toast("Elige cuentas distintas."); 
...
+"T00:00:00").getTime(),
     ...
     note: noteInp.value?.trim() || ""
      ...
      modal.close();
      U.toast(isEdit ? "Transferencia 

actualizada." : "Transferencia guardada.");
      await render();
    };
  }
...
it of (list||[])){
    // Soporta: transacciones "planas" (income/expense) o items mixtos {kind:"tx"/"tr"}
    if(it && 

it.kind==="tr"){
      const tr = it.tr;
      const fromN = accById(tr.fromAccountId)?.name || "";
      const toN = accById

(tr.toAccountId)?.name || "";
      rows.push([
        new Date(tr.dateMs).toLocaleDateString("es-ES"),
        "Transferencia",
        "",
...
"Gasto",
  ...
  // Totales (solo ingresos/gastos; transferencias no afectan al neto global)
  const {income, expense, 

balance} = sumPeriod(txOnly);
...
["Nota", "Exportado desde Control de Gastos"]
    ...
    return {
      filename: `ControlGastos_Panel_${label.replace(/\s

+/g,"_")}.xlsx`,
      sheets: [
        {name:"Resumen", rows: sheetResumen, currencyCols:[1]},
        {name:"Por categoría", rows: 

rowsByCategory(txList), currencyCols:[1]},
        {name:"Por cuenta (periodo)", rows: rowsByAccountPeriod(txList, trList), 

currencyCols:[1]},
        {name:"Movimientos", rows: rowsMovements(txList), currencyCols:[5]},
        {name:"Transferencias", 

rows: rowsTransfers(trList), currencyCols:[4]},
      ]
    };
  }
...
class="label">Ingresos</div><div class="value">${escapeHtml(U.money(sums.income, state.settings.currency))}

</div></div>
        <div class="kpi"><div class="label">Gastos</div><div class="value">${escapeHtml(U.money(sums.expense, 

state.settings.currency))}</div></div>
        <div class="kpi"><div class="label">Balance</div><div class="value">

${escapeHtml(U.money(sums.balance, state.settings.currency))}</div></div>
      </div>
    `;
    const byCatRows = 

rowsByCategory(txList).slice(1).map(r=>[r[0], U.money(r[1], state.settings.currency)]);
    const byAccRows = 

rowsByAccountPeriod(txList, trList).slice(1).map(r=>[r[0], U.money(r[1], state.settings.currency)]);
...
"<h3 style='margin:14px 0 8px'>Gasto por categoría</h3>" +
    ...
    "<h3 style='margin:14px 0 8px'>Variación por cuenta (periodo)</h3>" +
      tableHtml(["Cuenta","Variación"], byAccRows, 
...
new Set([1]));
  ...
  function printMovements(list=null, 

labelOverride=null){
  const label = labelOverride || periodLabel(currentPeriodRange());
...
(tr.fromAccountId)?.name || "";
      const toN = accById(tr.toAccountId)?.name || "";
      return [
        new Date

(tr.dateMs).toLocaleDateString("es-ES"),
        "Transferencia",
        "",
        `${fromN} → ${toN}`,
        tr.note || "",
...
(tr.amount, state.settings.currency)
      ];
    }
    const t = (it && it.kind==="tx") ? it.t : it;
    return [
...
tableHtml(["Fecha","Tipo","Categoría","Cuenta","Nota","Importe"], rows, new Set([5]));
  Print.printHtml

({title:"Movimientos", subtitle: label, html});
}
...
(c=>c.kind==="expense").map(c=>{
      const cap = ensureNum(state.settings.budgets?.[c.id]||0);
      const spent = 

ensureNum(exp.get(c.id)||0);
      const rem = cap ? (cap - spent) : 0;
      return [c.name, U.money(cap, state.settings.currency), 

U.money(spent, state.settings.currency), U.money(rem, state.settings.currency)];
    });
    const html = tableHtml

(["Categoría","Tope","Gastado","Restante"], rows, new Set([1,2,3]));
    Print.printHtml({title:"Presupuesto (Topes)", 

subtitle:`Periodo: ${label}`, html});
  }
...
const balances = await computeBalances();
    const rows = accounts().map(a=>[a.name, U.money(balances.get(a.id)||0, 

state.settings.currency)]);
    const html = tableHtml(["Cuenta","Saldo actual"], rows, new Set([1]));
    Print.printHtml

({title:"Cuentas", subtitle:`Saldo actual (histórico). Periodo visual: ${label}`, html});
  }
...
("div",{class:"grid cols2"},[
      U.el("div",{class:"kpi"},[U.el("div",{class:"label",text:"Ingresos"}), U.el("div",

{class:"value",text:U.money(sums.income, state.settings.currency)})]),
...
alertsNode, byCat, byAcc]);
  ...
  async function viewMovimientos(){
    setSubtitle("Movimientos (ingresos, gastos y 
...
dataTx = (state.cache.tx || []).slice();
    let dataTr = (state.cache.tr || []).slice();
    let rangeMode = "period"; // "period" o 
...
startDateInp = U.el("input",{class:"input", type:"date", value: toLocalISODate(range.start.getTime())});
    const endDateInp   

= U.el("input",{class:"input", type:"date", value: toLocalISODate(range.end.getTime() - 24*60*60*1000)});
    const 

applyRangeBtn = U.el("button",{class:"btn small", text:"Aplicar rango"});
    const resetRangeBtn = U.el("button",{class:"btn 

small", text:"Restablecer periodo"});

    async function applyCustomRange(){
      const sMs = fromISODateLocal

(startDateInp.value);
      const eMs0 = fromISODateLocal(endDateInp.value);
      if(!Number.isFinite(sMs) || !Number.isFinite

(eMs0)){
        UI.toast("Rango no válido. Revisa fecha inicio/fin.");
        return;
      }
      const eMs = eMs0 + 24*60*60*1000; // 

exclusivo
      if(eMs <= sMs){
        U.toast("Rango no válido: la fecha fin debe ser igual o posterior a la fecha inicio.");
        return;
    

  }
      rangeMode = "custom";
      dataTx = await DB.listTxByRange(state.db, sMs, eMs);
      dataTr = await DB.listTrByRange

(state.db, sMs, eMs);
      dataTx.sort((a,b)=>b.dateMs-a.dateMs);
      dataTr.sort((a,b)=>b.dateMs-a.dateMs);
      renderList();}
   

 function resetToPeriod(){
      rangeMode = "period";
      const r = currentPeriodRange();
      startDateInp.value = 

toLocalISODate(r.start.getTime());
      endDateInp.value = toLocalISODate(r.end.getTime() - 24*60*60*1000);
      dataTx = 

(state.cache.tx || []).slice();
      dataTr = (state.cache.tr || []).slice();
      renderList();
    }
    applyRangeBtn.addEventListener

("click", ()=>{ applyCustomRange(); });
    resetRangeBtn.addEventListener("click", ()=>{ resetToPeriod(); });
    ...
    const qInp = U.el("input",{class:"input", placeholder:"Buscar 

(nota/categoría)…"});
    const typeSel = U.el("select",{});
    [["all","Todos"],["income","Ingresos"],["expense","Gastos"],

["transfer","Transferencias"]].forEach(([v,t])=> typeSel.appendChild(U.el("option",{value:v,text:t})));
...
(){
  ...
  if(typeSel.value==="transfer"){
    catSel.appendChild(U.el("option",{value:"all", text:"(No aplica 
...
catSel.appendChild(U.el("option",{value:"all", text:"Todas las categorías"}));
  const kind = (typeSel.value==="all") ? null : 
...
refreshCatOptions();
    ...
    const transferBtn = U.el("button",{class:"btn small", text:"Transferir", onclick: ()=> 

openTransferModal({})});

    const addBtn = U.el("button",{class:"btn primary", text:"Añadir", onclick: ()=> 

openQuickAddModal()});
    const printB = printBtn(()=>{
      const sub = (rangeMode==="custom") ? `Rango: ${new Date

(fromISODateLocal(startDateInp.value)).toLocaleDateString("es-ES")} → ${new Date(fromISODateLocal

(endDateInp.value)).toLocaleDateString("es-ES")}` : `Periodo: ${label}`;
      printMovements(filteredList(), sub);
    });
    const 
...
`ControlGastos_Movimientos_${label.replace(/\s+/g,"_")}.xlsx`,
    ...
    const head = sectionHeader("Movimientos", U.el("div",{class:"row", style:"gap:8px"},[periodSwitcher(), printB, 
...
q = filters.q.trim();
  ...
  // Transacciones (Ingreso/Gasto)
  if(filters.type !== "transfer"){
  

  for(const t of dataTx){
      if(filters.type!=="all" && t.type!==filters.type) continue;
      if(filters.account!=="all" && 

(t.accountId||"cash")!==filters.account) continue;
      if(filters.category!=="all" && (t.categoryId||"")!==filters.category) 
...
filters.type==="transfer"){
    for(const tr of dataTr){
      if(filters.account!=="all"){
        const a = filters.account;
        const hit = 

(tr.fromAccountId||"cash")===a || (tr.toAccountId||"cash")===a;
        if(!hit) continue;
      }
      if(q){
        const fromN = accById
...
out.sort((a,b)=>{
    const ams = (a.kind==="tx") ? a.t.dateMs : a.tr.dateMs;
    const bms = (b.kind==="tx") ? b.t.dateMs : 
...
items = filteredList();
      listNode.innerHTML = "";
      if(!items.length){
        listNode.appendChild(U.el("div",{class:"card"}, 

U.el("div",{class:"tiny muted",text:"No hay movimientos con los filtros actuales."})));
        return;
      }
      for(const it of items){
...
t.type==="income" ? "pos" : "neg";
    ...
    const editBtn = U.el("button",{class:"btn 

small", text:"Editar", onclick: ()=> openTxModal({mode:"edit", tx:t})});
    const delBtn = U.el("button",{class:"btn small 

danger", text:"Borrar", onclick: async ()=>{
      const ok = await U.confirmDialog({title:"Borrar movimiento", message:"Se 

borrará este movimiento. ¿Quieres continuar?", okText:"Borrar", cancelText:"Cancelar"});
      if(!ok) return;
      await 

DB.delTx(state.db, t.id);
      U.toast("Movimiento borrado.");
      await render();
    }});

    listNode.appendChild(U.el("div",

{class:"item"},[
      U.el("div",{class:"left"},[
        U.el("div",{class:"title",text: `${cat || 
...
(t.type==="income"?"Ingreso":"Gasto")}`}),
      ...
      U.el("div",{class:"right"},[
        U.el("div",{class:`amount ${amtCls}`, text: `${amtSign}${U.money(t.amount, 
...
Date(tr.dateMs).toLocaleDateString("es-ES",{day:"2-digit",month:"short"});

    const editBtn = U.el("button",{class:"btn 

small", text:"Editar", onclick: ()=> openTransferModal({tr})});
    const delBtn = U.el("button",{class:"btn small danger", 

text:"Borrar", onclick: async ()=>{
      const ok = await U.confirmDialog({title:"Borrar transferencia", message:"Se borrará 

esta transferencia. ¿Quieres continuar?", okText:"Borrar", cancelText:"Cancelar"});
      if(!ok) return;
      await DB.delTr

(state.db, tr.id);
      U.toast("Transferencia borrada.");
      await render();
    }});
...
{class:"item"},[
      U.el("div",{class:"left"},[
        U.el("div",{class:"title",text: "Transferencia"}),
        U.el("div",

{class:"meta",text: `${date} · ${fromN} → ${toN}${tr.note? " · "+tr.note : ""}`})
      ]),
      U.el("div",{class:"right"},[
        U.el

("div",{class:"amount", text: U.money(tr.amount, state.settings.currency)}),
...
muted",text:"Categoría"}), catSel]),
    ...
    ,
      U.el("div",{class:"row", style:"gap:8px;flex-wrap:wrap;margin-top:10px"},[
      

  U.el("div",{style:"min-width:170px"},[U.el("div",{class:"tiny muted",text:"Fecha inicio"}), startDateInp]),
        U.el("div",

{style:"min-width:170px"},[U.el("div",{class:"tiny muted",text:"Fecha fin"}), endDateInp]),
        applyRangeBtn,
...
[{name:"Presupuesto", rows, currencyCols:[1,2,3]}]
        });
      })
    ]));
...
groupByCategoryExpenses(txList);
    ...
    const wrap = U.el("div",{class:"card"},[
      U.el("div",{class:"tiny muted", text:"Define 

un tope por categoría. La app avisará si lo superas."}),
      U.el("hr",{class:"sep"})
    ]);
...
style:"text-align:right"}),
      ...
      U.el("th",{text:"Restante", style:"text-
...
U.el("input",{class:"input", inputmode:"decimal", style:"text-align:right; max-width:140px"});
      capInp.value = cap ? 

U.number2(cap).replace(/\./g,"") : "";
...
= state.settings.budgets || {};
          ...
          else state.settings.budgets[cat.id] = 
...
await render();
      ...
      const tr = U.el("tr",{},[
        U.el("td",{},[
          U.el("div",{style:"font-weight:900"}, 
...
expCats = categories().filter(c=>c.kind==="expense");
...
está vacío o a 0, la categoría no tendrá límite."
    ));
...
periodLabel(range);
    ...
    const head = sectionHeader("Cuentas", U.el("div",
...
rows = [["Cuenta","Saldo actual"]];
        for(const a of accounts()) rows.push([a.name, ensureNum(balances.get(a.id)||0)]);
      

  XLSXMini.exportXLSX({ filename:`ControlGastos_Cuentas_${label.replace(/\s+/g,"_")}.xlsx`, sheets:[{name:"Cuentas", rows, 
...
{class:"h2", text:a.name}),
          U.el("span",{class:"badge", text:"Saldo"})
        ]),
        U.el("div",{style:"font-size:22px;font-

weight:900;margin-top:8px;font-variant-numeric:tabular-nums"}, U.money(bal, state.settings.currency)),
        U.el("div",

{class:"tiny muted", style:"margin-top:6px"}, "Incluye saldo inicial + histórico de movimientos y transferencias."),
        U.el

("hr",{class:"sep"}),
        U.el("button",{class:"btn small", text:"Ver extracto del periodo", onclick: ()=> 

openAccountExtractModal(a.id)})
      ]);
      cards.appendChild(card);
    }

    async function openAccountExtractModal
...
setTimeout(()=>{ URL.revokeObjectURL(a.href); a.remove(); }, 350);
    ...
    const impInp = U.el("input",{class:"input", 

type:"file", accept:"application/json,.json"});
    const impBtn = U.el("button",{class:"btn danger", text:"Importar JSON 

(restaurar)"});
    impBtn.onclick = async ()=>{
      const f = impInp.files?.[0];
      if(!f){ U.toast("Selecciona un archivo JSON."); 

return; }
      const ok = await U.confirmDialog({
        title:"Restaurar copia",
        message:"Se reemplazarán los datos actuales 

por los del backup. ¿Continuar?",
        okText:"Restaurar",
        cancelText:"Cancelar"
      });
      if(!ok) return;
...
= await f.text();
        ...
        if(!payload || !payload.settings || !Array.isArray(payload.tx) || !

Array.isArray(payload.tr)){
...
("Error importando JSON.");
    ...
    // Importar pegando texto JSON (útil en móvil cuando el selector de archivos no 
...
()=>{
      const ta = U.el("textarea",{class:"input", placeholder:"Pega aquí el contenido completo del backup JSON…", 

style:"min-height:220px;font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier 

New', monospace;white-space:pre;"});
      const tip = U.el("div",{class:"tiny muted", html:"Consejo: En el ordenador abre el 

.json, selecciona todo (Ctrl+A), copia (Ctrl+C) y envíalo por WhatsApp. En el móvil, copia el texto del chat y pégalo aquí."});
...
text:"Cancelar"});
      ...
      const modal = U.openModal({
        title:"Pegar JSON",
        contentNode: U.el("div",{class:"grid", 

style:"gap:10px"},[tip, ta]),
...
doBtn.onclick = async ()=>{
        ...
        if(!raw){ U.toast("Pega el contenido JSON."); 

ta.focus(); return; }

        const ok = await U.confirmDialog({
          title:"Restaurar copia",
          message:"Se reemplazarán los 

datos actuales por los del backup. ¿Continuar?",
          okText:"Restaurar",
          cancelText:"Cancelar"
        });
        if(!ok) return;
   ...
   try{
          ...
          if(!payload || !payload.settings || !Array.isArray(payload.tx) || !Array.isArray

(payload.tr)){
            U.toast("El texto no parece un backup válido.");
            return;
          }
          await DB.importAll(state.db, 
...
console.error(err);
    ...
    backupCard.appendChild(U.el("div",

{class:"grid cols2"},[
      U.el("div",{},[expBtn]),
      U.el("div",{},[impInp, U.el("div",{style:"margin-top:10px"}, [impBtn, U.el

("div",{style:"height:8px"}), pasteBtn])])
    ]));

    const wipeBtn = U.el("button",{class:"btn danger", text:"Borrar todos los 

datos"});
    wipeBtn.onclick = async ()=>{
      const ok = await U.confirmDialog({title:"Borrar todo", message:"Esto borrará 

todos los movimientos, transferencias y ajustes. Acción irreversible. ¿Continuar?", okText:"Borrar", cancelText:"Cancelar"});
...
text:"Versión: v1 · Datos en IndexedDB"}),
        wipeBtn
      ])
    ]);

    return U.el("div",{},[head, periodCard, themeCard, catCard, 

accCard, backupCard, footer]);
  }
...
appEl.innerHTML = "";
    ...
    try{
      ...
      else if(state.route==="presupuesto") node = 

await viewPresupuesto();
      else if(state.route==="cuentas") node = await viewCuentas();
      else if(state.route==="ajustes") 

node = await viewAjustes();
...
{class:"h1", text:"Error"}),
        U.el("div",{class:"tiny muted", style:"margin-top:8px"}, "Ha ocurrido un error renderizando la 

pantalla. Revisa la consola.")
      ]));
    }finally{
