...
// ---------- Period helpers ----------
  ...
  const {income, expense, balance} = sumPeriod(txOnly);
  ...
  function printMovements(list=null, labelOverride=null){
  const label = labelOverride || periodLabel(currentPeriodRange());
  const items = Array.isArray(list) ? list : (state.cache.tx || []);
  const rows = items.map(it=>{
    if(it && it.kind==="tr"){
      ...
      return [
        ...
        `${fromN} → ${toN}`,
        tr.note || "",
        U.money(tr.amount, state.settings.currency)
      ];
    }
    const t = (it && it.kind==="tx") ? it.t : it;
    return [
  ...
  Print.printHtml({title:"Movimientos", subtitle: label, html});
}
  ...
  async function viewMovimientos(){
    setSubtitle("Movimientos (ingresos, gastos y transferencias)");
    await loadPeriodData();

    const range = currentPeriodRange();
    const label = periodLabel(range);

    let dataTx = (state.cache.tx || []).slice();
    let dataTr = (state.cache.tr || []).slice();
    let rangeMode = "period"; // "period" o "custom"
    ...
    const startDateInp = U.el("input",{class:"input", type:"date", value: toLocalISODate(range.start.getTime())});
    const endDateInp   = U.el("input",{class:"input", type:"date", value: toLocalISODate(range.end.getTime() - 24*60*60*1000)});
    const applyRangeBtn = U.el("button",{class:"btn small", text:"Aplicar rango"});
    const resetRangeBtn = U.el("button",{class:"btn small", text:"Restablecer periodo"});

    async function applyCustomRange(){
      const sMs = fromISODateLocal(startDateInp.value);
      const eMs0 = fromISODateLocal(endDateInp.value);
      if(!Number.isFinite(sMs) || !Number.isFinite(eMs0)){
        UI.toast("Rango no válido. Revisa fecha inicio/fin.");
        return;
      }
      const eMs = eMs0 + 24*60*60*1000; // exclusivo
      if(eMs <= sMs){
      ...
      dataTr = await DB.listTrByRange(state.db, sMs, eMs);
    ...
    function resetToPeriod(){
      ...
      startDateInp.value = toLocalISODate(r.start.getTime());
    ...
    const typeSel = U.el("select",{});
    [["all","Todos"],["income","Ingresos"],["expense","Gastos"],["transfer","Transferencias"]].forEach(([v,t])=> typeSel.appendChild(U.el("option",{value:v,text:t})));
  ...
  if(typeSel.value==="transfer"){
    catSel.appendChild(U.el("option",{value:"all", text:"(No aplica a transferencias)"}));
...
refreshCatOptions();
    ...
    const printB = printBtn(()=>{
      const sub = (rangeMode==="custom") ? `Rango: ${new Date(fromISODateLocal(startDateInp.value)).toLocaleDateString("es-ES")} → ${new Date(fromISODateLocal(endDateInp.value)).toLocaleDateString("es-ES")}` : `Periodo: ${label}`;
    ...
    const head = sectionHeader("Movimientos", U.el("div",{class:"row", style:"gap:8px"},[periodSwitcher(), printB, exportB, transferBtn, addBtn]));
function matches(t, q){
  ...
  if(filters.type !== "transfer"){
    for(const t of dataTx){
      ...
      if(filters.account!=="all" && (t.accountId||"cash")!==filters.account) continue;
      if(filters.category!=="all" && (t.categoryId||"")!==filters.category) continue;
  ...
  // Transferencias
  if(filters.type==="all" || filters.type==="transfer"){
    for(const tr of dataTr){
      if(filters.account!=="all"){
        const a = filters.account;
        const hit = (tr.fromAccountId||"cash")===a || (tr.toAccountId||"cash")===a;
        if(!hit) continue;
      }
      if(q){
        const fromN = accById(tr.fromAccountId)?.name || "";
        const toN = accById(tr.toAccountId)?.name || "";
        const hay = (fromN+" "+toN+" "+(tr.note||"")).toLowerCase();
  ...
  out.sort((a,b)=>{
    ...
    const bms = (b.kind==="tx") ? b.t.dateMs : b.tr.dateMs;
    return bms - ams;
  });
...
const listNode = U.el("div",{class:"list"});
  ...
  if(it.kind==="tx"){
    ...
    const fromN = accById(tr.fromAccountId)?.name || "";
    const toN = accById(tr.toAccountId)?.name || "";
    const date = new Date(tr.dateMs).toLocaleDateString("es-ES",{day:"2-digit",month:"short"});

    const editBtn = U.el("button",{class:"btn small", text:"Editar", onclick: ()=> openTransferModal({tr})});
    const delBtn = U.el("button",{class:"btn small danger", text:"Borrar", onclick: async ()=>{
      const ok = await U.confirmDialog({title:"Borrar transferencia", message:"Se borrará esta transferencia. ¿Quieres continuar?", okText:"Borrar", cancelText:"Cancelar"});
      if(!ok) return;
      await DB.delTr(state.db, tr.id);
    ...
    listNode.appendChild(U.el("div",{class:"item"},[
      U.el("div",{class:"left"},[
        ...
        U.el("div",{class:"meta",text: `${date} · ${fromN} → ${toN}${tr.note? " · "+tr.note : ""}`})
      ]),
      U.el("div",{class:"right"},[
        U.el("div",{class:"amount", text: U.money(tr.amount, state.settings.currency)}),
        U.el("div",{class:"row", style:"gap:8px"},[editBtn, delBtn])
      ])
    ]));
  }
    ...
    ,
      U.el("div",{class:"row", style:"gap:8px;flex-wrap:wrap;margin-top:10px"},[
        ...
        U.el("div",{style:"min-width:170px"},[U.el("div",{class:"tiny muted",text:"Fecha fin"}), endDateInp]),
        applyRangeBtn,
        resetRangeBtn
      ])
    ]);
